...
export const TomiReveal: React.FC<TomiRevealProps> = ({ height, variant }) => {
  ...
  const RenderEditorial = () => (
    <>
      ...
      <div className="hidden md:flex absolute inset-0 flex-col p-12 pl-0">
        {/* Border Container */}
        <div className="w-full h-full border border-tomi-outline/60 border-l-0 relative flex flex-col rounded-r-full overflow-hidden bg-[#EEDCDC]">

          {/* Header Line & Badge */}
          <div className="w-full h-px bg-tomi-outline/40 mt-24 relative">
          ...
          <div className="flex-1 w-full flex relative overflow-hidden">
            {/* Left Side "to" */}
            <div className="w-[50vw] flex-none flex items-center justify-end pr-8 overflow-hidden border-r border-tomi-outline/40">
              <span
          ...
          {/* Footer Grid Info */}
          <div className="h-32 border-t border-tomi-outline/40 flex flex-row">
            <div className="w-[50vw] flex-none p-6 pl-[4.5rem] border-r border-tomi-outline/40 flex items-center">
  ...
  const RenderKinetic = () => (
    <>
      <div className="absolute inset-0 flex items-center justify-center overflow-hidden bg-[#EEDCDC]">

        {/* Abstract Background Circles */}
        <div
          className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[80vw] h-[80vw] border border-tomi-outline/5 rounded-full"
          style={{ transform: `translate(-50%, -50%) scale(${1 + (scrollY * 0.0005)}) rotate(${scrollY * 0.01}deg)` }}
        />
        <div
          className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[60vw] h-[60vw] border border-tomi-outline/5 rounded-full"
  ...
  return (
    <div
      className="fixed bottom-0 left-0 w-full z-0 bg-white flex flex-col justify-center items-center overflow-hidden"
      style={{ height }}
    >
